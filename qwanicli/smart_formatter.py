...
class SmartArabicFormatter:
    ...
    def format_verse(self, verse_data: Dict[str, Any], mode: str = 'auto', 
        ...
        if not arabic_text:
            click.echo(click.style("No Arabic text found in verse data", fg='yellow'))
            return
    ...
    def _display_transliteration(self, text: str) -> None:
        ...
        click.echo(click.style(f"[{transliterated}]", fg='yellow', italic=True))
        click.echo()
    ...
    def _display_verse_reference(self, verse_data: Dict[str, Any]) -> None:
        ...
        surah_name = self._extract_field(verse_data, ['surah_name', 'englishName'])
        surah_number = self._extract_field(verse_data, ['surah_number', 'number'])
        ayah_number = self._extract_field(verse_data, ['ayah_number', 'ayah', 'numberInSurah'])
        ...
        if surah_name or surah_number or ayah_number:
            ...
            click.echo(click.style(f"Reference: {reference}", fg='blue', bold=True))
    
    def _display_translation(self, verse_data: Dict[str, Any]) -> None:
        """Display verse translation if available."""
        translation = self._extract_field(verse_data, ['translation', 'text_translation', 'english'])
        translator = self._extract_field(verse_data, ['translator', 'edition', 'identifier'])
        ...
        if translation:
            ...
            import textwrap
            wrapped_translation = textwrap.fill(translation, width=80, initial_indent="  ", subsequent_indent="  ")
            click.echo(click.style(wrapped_translation, fg='white'))
            
            if translator:
                click.echo(click.style(f"  — {translator}", fg='magenta', dim=True))
    ...
    def display_font_recommendations(self) -> None:
        ...
        if self.arabic_support:
            click.echo(click.style("✅ Your terminal supports Arabic text display!", fg='green', bold=True))
            return
        
        click.echo(click.style("💡 For optimal Arabic text display, consider installing Arabic fonts:", fg='yellow', bold=True))
        click.echo()
        ...
        if platform_name == 'Linux':
            ...
            click.echo("  sudo apt install fonts-noto fonts-amiri fonts-scheherazade-new")
            click.echo()
            click.echo(click.style("Fedora/RHEL:", fg='cyan', bold=True))
            click.echo("  sudo dnf install google-noto-sans-arabic-fonts amiri-fonts")
            click.echo()
        elif platform_name == 'Darwin':  # macOS
            click.echo(click.style("macOS (with Homebrew):", fg='cyan', bold=True))
            click.echo("  brew install --cask font-noto-sans-arabic font-amiri font-scheherazade-new")
            click.echo()
        elif platform_name == 'Windows':
            click.echo(click.style("Windows:", fg='cyan', bold=True))
            click.echo("  Download fonts from Google Fonts or install via Microsoft Store")
            click.echo()
        
        click.echo(click.style("Recommended fonts:", fg='green', bold=True))
        fonts = ["• Noto Sans Arabic", "• Amiri", "• Cairo", "• Scheherazade New", "• DejaVu Sans"]
        for font in fonts:
            click.echo(f"  {font}")
